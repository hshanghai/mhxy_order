...
import { EXPENSE_TYPE_LABELS, INCOME_TYPE_LABELS, ExpenseType, IncomeType } from '../types';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white p-4 flex items-center justify-center">
        <div className="text-center">
          <p className="text-xl mb-4">人物不存在</p>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {(Object.entries(EXPENSE_TYPE_LABELS) as [ExpenseType, string][]).map(([type, label]) => (
              <Link
                key={type}
                to={`/character/${character.id}/expense/${type}`}
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-red-400 font-bold">¥{stats.expenseByType[type].toFixed(2)}</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {(Object.entries(INCOME_TYPE_LABELS) as [IncomeType, string][]).map(([type, label]) => (
              <Link
                key={type}
                to={`/character/${character.id}/income/${type}`}
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="font-semibold">{label}</span>
                  <span className="text-green-400 font-bold">¥{stats.incomeByType[type].toFixed(2)}</span>
                </div>
              </Link>
            ))}
          </div>
